...
public class Polyhedron extends Mesh {
    ...
    public Polyhedron clone() {
        ...
        for (Point3D vertexPos : vertexPositions) {
            clone.addVertexPosition(vertexPos);
        }
        for (Point3D normal : vertexNormals) {
            clone.addVertexNormal(normal);
        }
        for (Face face : faces) {
    ...
    public Polyhedron dual() {
        ...
        int vertexIndex = 0;
        for (Face face : faces) {
            Point3D cent = face.vertexAverage();
            double magnitude = cent.magnitude();
            cent = cent.multiply(1.0 / (magnitude * magnitude)); // Necessary for faces to be planar
        ...
        for (Face face : faces) {
            ...
            for (int i = 0; i < face.numVertices(); i++) {
                ...
                if (newVertices.get(edge) == null) {
                    Point3D edgeMidpt = edge.midpoint();
    ...
    public Polyhedron kis(int n) {
        ...
        for (Point3D vertexPos : vertexPositions) {
            kisPolyhedron.addVertexPosition(vertexPos);
        }
    ...
    public Polyhedron truncate(int n) {
        ...
        for (int i = 0; i < vertexPositions.size(); i++) {
            ...
            if (adjEdges.size() == n) {
                ...
                Point3D vertPos = vertexPositions.get(i);
                for (Edge edge : adjEdges) {
                    Point3D otherPos = edge.getOtherLocation(i);
                    Point3D newVert = VectorMath.interpolate(vertPos, otherPos,
    ...
    public Polyhedron gyro() {
        ...
        for (Point3D vertexPos : vertexPositions) {
            gyroPolyhedron.addVertexPosition(vertexPos);
        }
    ...
    public Polyhedron subdivide() {
        ...
        for (Point3D vertexPos : vertexPositions) {
            subdividePolyhedron.addVertexPosition(vertexPos);
        }
    ...
    private Polyhedron loft(int n, boolean ignore) {
        ...
        for (Point3D vertexPos : vertexPositions) {
            loftPolyhedron.addVertexPosition(vertexPos);
        }
        ...
        for (Face face : faces) {
            if (ignore || face.numVertices() == n) {
                ...
                for (int i = 0; i < face.numVertices(); i++) {
                    ...
                    Point3D vertex = vertexPositions.get(index);
                    Point3D newVertex = VectorMath.interpolate(vertex, centroid,
                                                               0.3);
    ...
    public Polyhedron quinto() {
        ...
        for (Point3D vertexPos : vertexPositions) {
            quintoPolyhedron.addVertexPosition(vertexPos);
        }
    ...
    private Polyhedron lace(int n, boolean ignore, boolean joined) {
        ...
        for (Point3D vertexPos : vertexPositions) {
            lacePolyhedron.addVertexPosition(vertexPos);
        }
    ...
    private Polyhedron stake(int n, boolean ignore) {
        ...
        for (Point3D vertexPos : vertexPositions) {
            stakePolyhedron.addVertexPosition(vertexPos);
        }
    ...
    public Polyhedron joinedMedial() {
        ...
        for (Point3D vertexPos : vertexPositions) {
            medialPolyhedron.addVertexPosition(vertexPos);
        }
    ...
    public Polyhedron propellor() {
        ...
        for (Point3D vertexPos : vertexPositions) {
            propellorPolyhedron.addVertexPosition(vertexPos);
        }
    ...
    public Polyhedron whirl() {
        ...
        for (Point3D vertexPos : vertexPositions) {
            whirlPolyhedron.addVertexPosition(vertexPos);
        }
        ...
        for (Face face : faces) {
            ...
            for (Edge edge : face.getEdges()) {
                ...
                int[] edgeVertices = newVertices.get(ends[0])
                                                .get(ends[1]);
                Point3D edgePoint = whirlPolyhedron.vertexPositions.get(edgeVertices[1]);
                Point3D diff = edgePoint.subtract(centroid)
                                        .multiply(0.3); // 0 < arbitrary scale factor < 1

                Point3D newFacePoint = centroid.add(diff);
