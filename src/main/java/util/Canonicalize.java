...
public class Canonicalize {
     ...
     */
    private static List<Point3D> reciprocalVertices(Polyhedron poly) {
        List<Point3D> newVertices = new ArrayList<>();

        List<Point3D> vertexPositions = poly.getVertexPositions();
        for (Face face : poly.getFaces()) {
            // Initialize values which will be updated in the loop below
            Point3D centroid = face.vertexAverage();
            ...
            for (int vertexIndex : faceVertexIndices) {
                Point3D vertex = vertexPositions.get(vertexIndex);

                // Compute the normal of the plane defined by this vertex and
                // the previous two
                Point3D lastlastVertex = vertexPositions.get(lastLastVertexIndex);
                Point3D lastVertex = vertexPositions.get(lastVertexIndex);
                Point3D v1 = lastlastVertex.subtract(lastVertex);
                Point3D v2 = vertex.subtract(lastVertex);
                Point3D normal = v1.crossProduct(v2);
                normalSum = normalSum.add(normal);

                // Compute distance from edge to origin
                avgEdgeDistance += Geometry.pointLineDist(new Point3D(0, 0, 0),
                                                          lastlastVertex,
                                                          lastVertex);
            ...
            Point3D resultingVector = normalSum.multiply(centroid.dotProduct(normalSum));
            double magnitude = resultingVector.magnitude();
            resultingVector = resultingVector.multiply(1.0 / (magnitude
                                                              * magnitude));
            resultingVector = resultingVector.multiply((1.0 + avgEdgeDistance)
                                                       / 2.0);
    ...
    public static void planarize(Polyhedron poly, int numIterations) {
        ...
        for (int i = 0; i < numIterations; i++) {
            List<Point3D> newDualPositions = reciprocalVertices(poly);
            dual.setVertexPositions(newDualPositions);
            List<Point3D> newPositions = reciprocalVertices(dual);

            double maxChange = 0.;
            for (int j = 0; j < poly.getVertexPositions()
                                    .size(); j++) {
                Point3D newPos = poly.getVertexPositions()
                                     .get(j);
                Point3D diff = newPos.subtract(poly.getVertexPositions()
                                                   .get(j));
     ...
     */
    private static List<Point3D> reciprocalCenters(Polyhedron poly) {
        List<Point3D> faceCenters = new ArrayList<>();
        for (Face face : poly.getFaces()) {
            Point3D newCenter = face.vertexAverage();
            double magnitude = newCenter.magnitude();
            faceCenters.add(newCenter.multiply(1.0 / (magnitude * magnitude)));
        }
        return faceCenters;
    }
    ...
    public static void adjust(Polyhedron poly, int numIterations) {
        Polyhedron dual = poly.dual();
        for (int i = 0; i < numIterations; i++) {
            List<Point3D> newDualPositions = reciprocalCenters(poly);
            dual.setVertexPositions(newDualPositions);
            List<Point3D> newPositions = reciprocalCenters(dual);
            poly.setVertexPositions(newPositions);
        }
        poly.setVertexNormalsToFaceNormals();
    }
    ...
    private static int canonicalize(Polyhedron poly, double threshold,
                                    boolean planarize) {
        Polyhedron dual = poly.dual();
        List<Point3D> currentPositions = Struct.copyVectorList(poly.getVertexPositions());

        int iterations = 0;
        while (true) {
            List<Point3D> newDualPositions = planarize ? reciprocalVertices(poly)
                                                       : reciprocalCenters(poly);
            dual.setVertexPositions(newDualPositions);
            List<Point3D> newPositions = planarize ? reciprocalVertices(dual)
            ...
            for (int i = 0; i < currentPositions.size(); i++) {
                Point3D newPos = poly.getVertexPositions()
                                     .get(i);
                Point3D diff = newPos.subtract(currentPositions.get(i));
                maxChange = Math.max(maxChange, diff.magnitude());
            }
